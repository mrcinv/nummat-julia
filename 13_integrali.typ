...
kako izbrati primerno metodo, glede na funkcijo, ki jo integriramo.
...
- Implementiraj sestavljeno trapezno, sestavljeno Simpsonovo, Gauss-Legendrove kvadrature in
  adaptivno Simpsonovo metodo. Za izračun vozlov in uteži Gauss-Legendrovih kvadratur uporabi paket
...
razbijemo na vsoto integralov po intervalih $[x_(i), x_(i+1)]$ in na vsakem podintervalu
...
$
  integral_a^b f(x) d x &approx  h/2(f(a)+f(x_1)) + h/2(f(x_1)+f(x_2)) + dots\
  dots & + h/2(f(x_(n-2)) + f(x_(n-1))) + h/2 (f(x_(n-1)) + f(b))=\
...
Določeni integral ima dve sestavini: funkcijo in interval. Tako bo imel podatkovni tip
...
in so posledica zamenjave podatkov iste oblike, ki nastopajo v različnih vlogah.

Ko smo določili zapis integrala, moramo razmisliti, kako v programu predstaviti sestavljeno trapezno
pravilo. V objektno usmerjenih programskih
...
jo integriramo. Izpeljavo Simpsonovega pravila si lahko ogledate v @sec:13-izpeljava[podpoglavju].

Podobno kot trapezno lahko tudi Simpsonovo pravilo preoblikujemo v sestavljeno pravilo. Sestavljeno
Simpsonovo pravilo z $2n$ enakomernimi koraki dolžine $h$ je dano kot:
...
Vrednostim $u_k$ pravimo #emph[uteži], $x_k$ pa so #emph[vozli] kvadraturne formule. Za sestavljeno
trapezno formulo so uteži enake $u_0 = u_n = h/2$ in $u_k = h$ za $1<= k<=n-1$,
vozli pa so $x_k = a + k h$ za $k=0, 1, med dots, med n$.
...
dobimo #link("https://en.wikipedia.org/wiki/Gauss%E2%80%93Legendre_quadrature")[Gauss-Legendrove]
kvadrature
...
Gauss-Legendrovo kvadraturo z $n$ vozli so ničle Legendrovega
polinoma stopnje $n$ optimalna izbira. Gauss-Legendrova kvadratura z $n$ vozli je točna
za polinome stopnje $2n - 1$. Za $n=1$ dobimo sredinsko pravilo z vozlom v $x_1=0$, ki je točno
...
Če želimo Gauss-Legendrovo kvadraturo uporabiti na poljubnem intervalu $[a, b]$,
...
Pri trapeznem in Simpsonovem pravilu smo uteži in vozle računali
...
Gauss-Legendrovih kvadraturah pa je izračun vozlov in uteži precej zahtevnejši, zato jih je smiselno shraniti za večkratno uporabo. Kvadrature bomo obravnavali splošno in definirali
...
utežmi $u_1, u_2, med dots, med u_n$ in intervalom $[a, b]$.
#naloga[
  Definiraj podatkovni tip #jl("Kvadratura") s tremi polji:
- #jl("x") vektor vozlov,
- #jl("u") vektor uteži in
- #jl("interval") interval.
Rešitev je @pr:13-Kvadratura.
]
...
za kvadraturo. Za primer bomo uporabili Gauss-Legendrovo kvadraturo z dvema vozloma @eq:13-gl2,
ki je točna za kubične polinome.


#naloga[
  - Napiši test za integracijo s kvadraturo tipa #jl("Kvadratura") (@pr:13-kvad-test).
  - Napiši metodo $jl("integriraj(i::Integral, k::Kvadratura)")$, ki izračuna približek za dani integral #jl("i") z dano kvadraturo #jl("k") (@pr:13-int-gl).
  - Napiši test za funkcijo, ki vrne Gauss-Legendrove kvadrature.
    Uporabi dejstvo, da so kvadrature z $n$ vozli točne za polinome stopnje $2n-1$ (@pr:13-glkvad-test).
  - Napiši funkcijo #jl("glkvad(n::Int)"), ki vrne Gauss-Legendrovo
    kvadraturo z $n$ vozli (@pr:13-glkvad).
]
...
Preslikava $I_([a, b])$ je linearni funkcional, zato želimo, da je tudi kvadratura $K(f)$ linearni
...
Kako vemo, kateri rezultat je natančnejši? Točnega rezultata ne znamo izračunati, saj integral
@eq:13-sinx2 ni elementaren. Nekako moramo oceniti napako. Uveljavili sta se dve strategiji za oceno napake. Za sestavljene kvadraturne formule rezultat primerjamo z rezultatom iste
...
Vidimo, da se je najbolje odrezala Gauss-Legendrova kvadratura. Oglejmo si na grafu, kako absolutna vrednost napake pojema
 s številom izračunanih funkcijskih vrednosti za vse tri metode.
...
funkcije. V tem primeru lahko vrednost integrala izračunamo:
...
#opomba(naslov: [Kaj smo se naučili?])[
  - Kvadraturne formule se večinoma prevedejo na uteženo vsoto funkcijskih vrednostih v
    vozlih kvadrature.
  - Gauss-Legendrove kvadrature so najprimernejše za integrale funkcij, ki jih lahko dobro
...
#test13("# gl")[Test za generiranje Gauss-Legendrovih kvadratur]<pr:13-glkvad-test>
...
vrednost funkcije na sredini. Pravilo zapišemo v naslednji obliki:

$
  integral_0^h f(x) d x = A f(a) + B f((a + b)/2) + C f(b) + R_f,
$

kjer so $A$, $B$ in $C$ uteži ter $R_f$ razlika med pravo vrednostjo in približkom. Uteži
...
$
integral_0^h x^4 d x = h^5/5 =  (2h)/3 dot.c h^4/16 + h/6 dot.c h^4 + R_(x^4) = (5h^5)/(24) + R_(x^4)\
R_(x^4) = h^5/5 - (5h^5)/24 = -1/(120) h^5.
$

Ker je $x^4$ najnižja stopnja polinoma, pri kateri napaka vedno enaka $0$, bo v formuli za napako
nastopala vrednost četrtega odvoda $f^((4))(xi)$ v neznani točki $xi$. Napako zapišemo kot
$R_f = C h^5 f^((4))(xi)$ in:
